...
function NotificationItem({
  ...
  return (
    <button
      ...
      className={cn(
        "flex w-full gap-3 border-l-2 px-3 py-3 text-left transition-colors hover:bg-accent/50",
        isUnread
...
export function NotificationsBell({
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        () => {
          // Refetch to get latest counts and read states
          refresh();
        }
      )
      .subscribe();
  ...
  const handleItemClick = async (n: NotificationRow) => {
    ...
    if (!n.is_read) {
      ...
      // Server update (no need to await for navigation)
